 ...
 * @param {*} config   User Configurations from JSON url input
 * @param {*} infoDivExpand  ArcGIS Expand widget
 * @param {*} view ArcGIS MapView
 */
 ...
 * @param {*} basemapGallery ArcGIS BasemapGallery widget
 * @param {*} bgExpand ArcGIS Expand widget
 * @param {*} view ArcGIS MapView
 */
...
function loadPopUpTemplate(config) {
  ...
  let formatContent = function (feature) {
    if (
      config.popupTemplateMap.has(feature.graphic.attributes.tlcMapUniqueId) &&
      config.popupTemplateMap.get(feature.graphic.attributes.tlcMapUniqueId)
        .content != null &&
      config.popupTemplateMap.get(feature.graphic.attributes.tlcMapUniqueId)
        .content != ""
    ) {
 ...
 *  @param {*} config User Configurations from JSON url input
 * @param {*} layerLlistExpand ArcGIS Expand widget
 * @param {*} view ArcGIS MapView
 */
function loadListPane(config, layerListExpand, view , layerList){
  ...
  layerListExpand.expanded = config.listPane === "hidden" ? false : true,
