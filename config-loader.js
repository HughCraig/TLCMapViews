    ...
    return new Promise((resolve, reject) => {
      fetch(urltoload)
        ...
        .catch((err) => {
          console.error(err);
          config.titleText =
            "Error: Unable to load GeoJSON data from the specified URL";
        ...
        if (info.hasOwnProperty("title")) {
          if (typeof info.title === "string") {
            ...
            if (info.title.target) {
              config["titleLinkTarget"] = info.title.target;
            }
          }
        }
      ...
      if (display.hasOwnProperty("popup")) {
        ...
        if (
          ...
          if (
            ...
            // case for ["*"]
            config["popupAllowedFields"] = new Set(display.popup.allowedFields);
          }
        }
    ...
    if (data.features) {
      data.features.forEach((feature, index) => {
        ...
        if (feature.display && feature.display.popup) {
          ...
          //Links
          if (popUp.hasOwnProperty("links") && Array.isArray(popUp.links)) {
            let links = [];
